...
class TestLogin:
    ...
    def test_login_with_wrong_password(self, page: Page, test_config):
        ...
        if error_message:
            ...
            # Если нет сообщения об ошибке, проверяем что мы остались на странице логина
    ...
    def test_empty_credentials(self, page: Page, test_config):
        ...
        # Оставляем поля пустыми и жмем войти
        login_page.click_login_button()

        # Проверяем валидацию
        # В зависимости от реализации, могут быть разные индикаторы
        assert not login_page.is_login_successful(), \
